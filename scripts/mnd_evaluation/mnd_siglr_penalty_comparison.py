...
SEED = 42
RESULT_FILE = "results/mnd/penalty_comparison.parquet"

# ========== MODELS ==========
DEGREE = 2

MODELS_DICT: dict[str, BaseEstimator] = {
    "mnd_siglr_penalty_lasso": SigLR(
        DEGREE, time_aug=True, lead_lag_aug=True, l1_ratio=1.0, random_state=SEED
    ),
    "mnd_siglr_penalty_ridge": SigLR(
        DEGREE, time_aug=True, lead_lag_aug=True, l1_ratio=0.0, random_state=SEED
    ),
    "mnd_siglr_penalty_elastic": SigLR(
        DEGREE, time_aug=True, lead_lag_aug=True, l1_ratio=0.5, random_state=SEED
    ),
}
